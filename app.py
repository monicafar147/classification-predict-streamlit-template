...
def main():
	...
	print("linear SVC model successfully pickled")
	...
	print('loading process function')
	...
	if selection == "Home":
		...
		if st.button("Classify LR model"):
			...
			st.success("Text Categorized as: {}".format(tweet_pred))
		# add choose a model button
		if st.button("Classify SVC grid search model"):
			tweet = tweet_text
			print("input tweet : \n {}".format(tweet))
			tweet_processed = cleaner(tweet)
			print("processed tweet : \n {}".format(tweet_processed))
